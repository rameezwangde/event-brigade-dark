...
function HexButton({ href, label, Icon }) {
  return (
    <a
      ...
      rel="noreferrer"
      aria-label={label}
      className="grid h-9 w-9 place-items-center text-ivory/75 transition hover:text-gold"
    >
...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#050505] text-ivory">
      <div className="mx-auto max-w-7xl px-6 py-16 md:px-10 lg:px-14">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4 lg:gap-14">
          <div>
            ...
            <p className="mt-6 max-w-xs text-sm leading-7 text-ivory/70">
          ...
          <div>
            <h3 className="font-serif text-2xl font-bold text-ivory">Navigation</h3>
            <div className="mt-6 grid gap-3">
              {quickLinks.map(([label, id]) => (
                <a key={label} href={`#${id}`} className="block text-sm text-ivory/70 transition hover:text-gold">
                  {label}
                </a>
              ))}
            </div>
          ...
          <div>
            <h3 className="font-serif text-2xl font-bold text-ivory">Services</h3>
            <div className="mt-6 grid gap-3">
              {serviceLinks.map((service) => (
                <a key={service} href="#services" className="block text-sm text-ivory/70 transition hover:text-gold">
                  {service}
                </a>
              ))}
            </div>
          ...
          <div>
            <h3 className="font-serif text-2xl font-bold text-ivory">Contact</h3>
            <div className="mt-6 space-y-4 text-sm leading-7 text-ivory/70">
              <a
                href={contact.mapUrl}
                target="_blank"
                rel="noreferrer"
        ...
        <div className="mt-14 border-t border-ivory/15 pt-7">
          <div className="flex flex-col gap-5 text-xs text-ivory/60 md:flex-row md:items-center md:justify-between">
